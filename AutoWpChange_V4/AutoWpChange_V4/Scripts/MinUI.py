...
"}")
    ...
    def AddToStartup(self) -> str:
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_SET_VALUE) as reg_key:
...
版权：思锐工作室'''
    ...
    def r(self):
         if os.path.exists(".\\RefreshSetting.Sr"):
            ...
            if "AutoStart" in user_setting[0]:
                self.CheckBox_5.setChecked(True)
            if "AutoChange" in user_setting[0]:
                self.CheckBox_6.setChecked(True)

            if "PC" in user_setting[len(user_setting)-1]:
                print(1)
                self.RadioButton_9.setChecked(True)
            elif "Starry" in user_setting[len(user_setting)-1]:
                print(2)
                self.RadioButton_10.setChecked(True)
            elif "Top" in user_setting[len(user_setting)-1]:
                print(3)
